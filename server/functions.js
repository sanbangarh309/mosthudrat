...
module.exports = {
  ...
  sanGenerateQRCode : function(req, res,id, callback) {
    ...
    var realpath = 'http://'+req.headers.host+'/files/qrcodes/'+id+'.png';
    var path = config.directory+'/uploads/qrcodes/'+id+'.png';
  ...
  sanGenerateBarCode : function(id,sb) {
    ...
    bwipjs.toBuffer({
        ...
        if (err) {
          ...
          require('fs').writeFile(config.directory+'/uploads/qrcodes/'+id+'.png', png,
            function()
            {
              sb(id+'.png');
            });
